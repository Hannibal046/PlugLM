...
import transformers
from question_answering.trainer_qa import QuestionAnsweringTrainer
...
from question_answering.utils_qa import postprocess_qa_predictions
from utils import (
    is_main_process,
    update_args,
)
from modeling import (
    RetrievalBertConfig,
    RetrievalBertForQuestionAnswering,
)
...
class ModelArguments:
    ...
    keys_path:str = field(default=None,)
    values_path:str = field(default=None,)
    model_name_or_path:Optional[str] = field(default="../pretrained_model/bert_base_uncased",)
    values_device:str=field(default="cuda")
    retrieval_bert:bool = field(default=False)
    fix_keys:bool = field(default=False)
    fix_values:bool = field(default=False)
...
def main():
    ...
    if not model_args.retrieval_bert:
        ...
        model = RetrievalBertForQuestionAnswering(pretrain_config)
        pretrained_state_dict = torch.load(model_args.ckpt_path)
        ...
        model.update_memory(keys,values,model_args.values_device)

        tokenizer = transformers.BertTokenizerFast(model_args.vocab_path)
    ...
    metric = load_metric("./metrics/squad_v2/squad_v2.py" if data_args.version_2_with_negative else "./metrics/squad/squad.py",
                         cache_dir = model_args.cache_dir)
