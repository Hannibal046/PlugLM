...
def load_retrieval_bert(ckpt_dir,num_labels=2):
    ## import 
    from modeling import (
        RetrievalBertConfig,
        RetrievalBertForSequenceClassification,
        RetrievalBertForYesno,
    )
    ...
    keys_path = os.path.join(ckpt_dir,'additional_keys_and_values','10_30_pubmed','keys.npy')
    values_path = os.path.join(ckpt_dir,'additional_keys_and_values','10_30_pubmed','values.npy')
    ...
    config.num_labels = num_labels
    model = RetrievalBertForYesno(config).to(device)
...
def evaluate(args, model, tokenizer, prefix=""):
    t = torch.load("/mnt/wfs/mmchongqingwfssz/user_mqxincheng/mybert/data/bio/QA/BioASQ/biomed_qa_yesno_dev_bert_uncased")
    dataset = t['dataset']
    examples = t['examples']
    features = t['features']
...
def main():
    ...
    model,tokenizer = load_retrieval_bert(args.model_name_or_path,num_labels=2)
    ...
    # Training
    if args.do_train:
        train_dataset = torch.load("/mnt/wfs/mmchongqingwfssz/user_mqxincheng/mybert/data/bio/QA/BioASQ/biomed_qa_yesno_train_bert_uncased")['dataset']
