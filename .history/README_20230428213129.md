...
The key intuition is to decouple the knowledge storage from model parameters(Feed-Forward Network, where the knowledge is stored) with an editable and scalable key-value memory and leverage knowledge in an explainable manner by knowledge retrieval in the DPM. 
