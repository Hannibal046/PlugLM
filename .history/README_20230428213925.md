...
Inspired by the findings that Feed-forward Network in PLM stores all kinds of knowledge and is essentially a key-value memory network, we propose **PlugLM**, the first PLM to d knowledge storage from model parameters with an editable and scalable key-value memory and making the knowledge encoding of PLMs more flexible and interpretable. 
