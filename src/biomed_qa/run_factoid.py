...
from src.modeling import RetrievalBertForQuestionAnswering, RetrievalBertForSequenceClassification
...
def load_retrieval_bert(ckpt_dir,num_labels=2):
    ...
    config.num_labels = num_labels
    model = RetrievalBertForQuestionAnswering(config).to(device)
...
def main():
    ...
    #     cache_dir=args.cache_dir if args.cache_dir else None,
    # )
    model,tokenizer = load_retrieval_bert(args.model_name_or_path)
